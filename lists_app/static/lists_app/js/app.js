/*
    JS for `lists_app/static/lists_app/js/app.js`

    Sections:
    1. Angular module + config
    2. Controller init
    3. Drag & Drop handlers
    4. Item/Section reordering helpers
    5. WebSocket setup + message handling
    6. CRUD operations (add/update/delete/toggle)
    7. Sections management + grouping
    8. Helpers (send, snapshots)
    9. Theme + init
...
angular
  ...
  .controller("MainCtrl", [
    ...
    function ($scope, $timeout, $http) {
      const vm = this;
      // Toggle verbose debugging logs without removing statements.
      const DEBUG = false;
      vm.lists = [];
      vm.currentList = null;
      vm.items = [];
      ...
      vm.initDragAndDrop = function () {
        document.addEventListener("dragstart", function (e) {
          if (DEBUG) console.log("Drag start on:", e.target);
          ...
          if (e.target.classList.contains("drag-handle")) {
            ...
            if (handle.classList.contains("item-drag-handle")) {
              ...
              if (DEBUG) console.log("Found item drag handle for ID:", itemId);
        ...
        document.addEventListener("dragend", function (e) {
          ...
          if (e.target.classList.contains("drag-handle")) {
            ...
            document
              .querySelectorAll(".drag-over, .section-drag-over")
              .forEach((el) => {
          ...
          if (vm.draggedItem || vm.draggedSection) {
            ...
            while (targetElement && targetElement !== document) {
              ...
              if (
                ...
                targetElement.hasAttribute("data-item-id")
              ) {
                ...
                if (item && vm.draggedItem && item.id !== vm.draggedItem.id) {
                  ...
                  if (DEBUG) console.log("Drag over item:", item.name);
                  break;
                }
              }
              // Check if it's a section container
                ...
                targetElement.classList &&
                targetElement.classList.contains("section-container")
              ) {
                const sectionName =
                  targetElement.getAttribute("data-section-name");
                if (
                  vm.draggedSection &&
                  sectionName !== vm.draggedSection &&
                  sectionName !== vm.noneSectionName
                ) {
                  vm.dragOverSection = sectionName;
                  targetElement.classList.add("section-drag-over");
                  if (DEBUG) console.log("Drag over section:", sectionName);
                  break;
        ...
        document.addEventListener("drop", function (e) {
          ...
          if (vm.draggedItem && vm.dragOverItem) {
            ...
            if (DEBUG)
              console.log(
                "Dropping item on item:",
                vm.draggedItem.name,
                "->",
                vm.dragOverItem.name,
              );
            ...
            if (DEBUG)
              console.log(
                "Item dropped in empty space, looking for section...",
              );
            let targetElement = e.target;
              ...
              if (
                ...
                if (DEBUG)
                  console.log(
                    ...
                    targetSection ? targetSection.name : "none",
                  );
                break;
              }
              targetElement = targetElement.parentElement;
            ...
            if (targetSection) {
              const currentSectionName = vm.draggedItem.section
                ? vm.draggedItem.section.name
                : vm.noneSectionName;
              const targetSectionName = targetSection
                ? targetSection.name
                : vm.noneSectionName;
              ...
              if (currentSectionName !== targetSectionName) {
                if (DEBUG) console.log("Moving item to different section");
                vm.moveItemToSection(vm.draggedItem, targetSection);
              } else {
                if (DEBUG) console.log("Item already in target section");
              }
            } else {
              if (DEBUG) console.log("No target section found");
            }
          } else {
            if (DEBUG) console.log("No valid drop target found");
          }
      ...
      vm.handleItemDrop = function (draggedItem, targetItem) {
        ...
        const draggedSectionName = draggedItem.section
          ? draggedItem.section.name
          : vm.noneSectionName;
        const targetSectionName = targetItem.section
          ? targetItem.section.name
          : vm.noneSectionName;
        ...
        if (draggedSectionName !== targetSectionName) {
          ...
          if (DEBUG) console.log("Reordering items within same section");
          vm.reorderItemInSection(draggedItem, targetItem);
        }
      };

      vm.reorderItemInSection = function (draggedItem, targetItem) {
        const sectionName = draggedItem.section
          ? draggedItem.section.name
          : vm.noneSectionName;
      ...
      vm.handleSectionDrop = function (draggedSectionName, targetSectionName) {
        if (
          !draggedSectionName ||
          !targetSectionName ||
          draggedSectionName === targetSectionName
        )
          return;

        if (DEBUG)
          console.log(
            "Handling section drop:",
            draggedSectionName,
            "->",
            targetSectionName,
          );

        const draggedIndex = vm.sections.findIndex(
          (s) => s.name === draggedSectionName,
        );
        const targetIndex = vm.sections.findIndex(
        ...
        if (DEBUG)
          console.log(
            "Swapping section orders:",
            draggedSection.name,
            draggedSection.order,
            "<=>",
            targetSection.name,
            targetSection.order,
          );
      ...
      vm.moveItemToSection = function (item, newSection) {
        ...
        const oldSectionName = item.section
          ? item.section.name
          : vm.noneSectionName;
        ...
        if (DEBUG) console.log("New order for item:", newOrder);
      ...
      vm.connect = function (listId) {
        ...
        vm.socket.onmessage = (evt) =>
          $scope.$apply(() => handleMessage(JSON.parse(evt.data)));
      };
      ...
      function handleMessage(msg) {
        ...
        if (msg.action === "sections_reordered") {
          ...
          if (DEBUG) console.log("Received sections reordered:", msg.sections);
          msg.sections.forEach((updatedSection) => {
            const existingSection = vm.sections.find(
      ...
      vm.commitUpdate = function (it) {
        ...
        const oldSectionName =
          old && old.section ? old.section.name : vm.noneSectionName;
        const newSectionName = it.section
          ? it.section.name
          : vm.noneSectionName;
      ...
      vm.handleBlur = function () {
        if (vm.newItem.name.trim() !== "") vm.addNewItem();
      };
      ...
      vm.addNewItem = function () {
        ...
        const secName = vm.newItem.section
          ? vm.newItem.section.name
          : vm.noneSectionName;
        ...
        if (list.length > 0) {
          const max = Math.max.apply(
            null,
            list.map((it) => (typeof it.order !== "undefined" ? it.order : 0)),
          );
          nextOrder = max + 1;
        }
